...
/**
 * Advanced usage example showing custom configuration, streaming, and error handling.
 *
 * This example demonstrates:
 * - Custom OkHttpClient configuration (timeouts, proxies, etc.)
 * - Custom client options (timeout, retries, logging)
...
public class AdvancedUsage {
    ...
    private static void structuredMessage(String prompt) {
        try (AnthropicClient client = AnthropicOkHttpClient.fromEnv()) {
            MessageCreateParams params = MessageCreateParams.builder()
                .model(Model.CLAUDE_SONNET_4_20250514)
                .maxTokens(1024)
                .temperature(0.7f) // More creative responses
                .topP(0.9f) // Use nucleus sampling
                .addUserMessage(prompt)
                .build();
    ...
    private static void useDifferentModels() {
        try (AnthropicClient client = AnthropicOkHttpClient.fromEnv()) {
            // Using different available models
            Model[] models = {
